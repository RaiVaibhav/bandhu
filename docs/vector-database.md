...
**Model availability is account/region-specific — confirmed 2026-07-14.** Meta's Llama 3.1 family, the original pick, wasn't actually available on this NVIDIA account. What was available instead: Qwen3.5 (`397b-a17b` / `122b-a10b`), Qwen3-Next (`80b-a3b-instruct`), and DeepSeek V4 (`flash` / `pro`). Worth checking your own account's actual catalog before assuming any specific model ID works — NIM's free-tier availability isn't uniform.

### Generation model tiers — a recommendation, not a default

The project's own `bandhu-research-resources.md` already set the principle: *"Use a fast/cheap model for the phrasing step... reserve the tone evaluator for a sampled percentage."* That's a real architectural reason to use different model tiers for different pipeline stages — not cost-cutting for its own sake. Concretely, mapped onto `pipeline.html`'s stages, using the NVIDIA NIM models actually confirmed available — picked by active-parameter size within each family (NIM's MoE models are named `<total>b-a<active>b`, e.g. `80b-a3b` = 80B total, 3B active) rather than staying within one model family:


| Stage                        | What it needs                                                                  | Model (NVIDIA NIM)                | Why                                                                                                                                 |
| ----------------------------- | ------------------------------------------------------------------------------ | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Stage 7 — Orchestrator       | Judgment: eligible or not, which tool, does this feel right                    | `deepseek-ai/deepseek-v4-pro`    | DeepSeek's flagship reasoning tier — this is the one place in the whole pipeline with real discretion, worth spending the best available model here specifically |
| Stage 8 — Generate           | Phrasing only, constrained to retrieved content, ~60-word cap                  | `qwen/qwen3-next-80b-a3b-instruct` | Smallest active-parameter count of what's available — compose, don't decide. This is the "reserve the smart model for judgment, not phrasing" principle in practice |
| Stage 11 — Summarizer        | Synthesize structured facts into a short narrative                             | `qwen/qwen3.5-122b-a10b`         | Mid-sized — more than pure phrasing, less than live judgment, periodic and async so latency matters less than on stages 7/8         |
| Stage 12 — Sampled evaluator | Score against the MITI rubric — needs real judgment to be a trustworthy signal | `deepseek-ai/deepseek-v4-pro`    | Only runs on 5-10% of responses, so cost is already controlled by the sampling rate, not by using a cheaper judge                   |


This is a starting recommendation, not a locked decision — model choice is genuinely yours to make per stage, and the right call may shift once there's real usage data on cost and quality, or once a paid Claude/GPT key becomes affordable and worth comparing against. The one part of this that isn't a preference: don't use the smallest/fastest model for the Orchestrator or the Evaluator — those are exactly the two places the pipeline is relying on judgment, and that's specifically what the small model trades away.
